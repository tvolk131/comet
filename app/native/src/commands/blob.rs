...
pub fn get_blossom_url(app: AppContext) -> Result<Option<String>, AppError> {
    let conn = database_connection(&app)?;
    Ok(crate::adapters::sqlite::sync_settings_repository::get_blossom_url(&conn))
}

pub fn set_blossom_url(app: AppContext, url: String) -> Result<(), AppError> {
    let parsed =
...
pub fn remove_blossom_url(app: AppContext) -> Result<(), AppError> {
    let conn = database_connection(&app)?;
    conn.execute("DELETE FROM app_settings WHERE key = 'blossom_url'", [])?;
    Ok(())
}

pub async fn fetch_blob(app: AppContext, hash: String) -> Result<BlobFetchStatus, AppError> {
    ...
    if !crate::adapters::native::key_store::is_current_identity_unlocked(&app, &conn)? {
        log::info!("[blob] needs unlock plaintext_hash={hash}");
        return Ok(BlobFetchStatus::NeedsUnlock);
    }

    let (keys, pubkey_hex) =
        crate::adapters::native::key_store::keys_for_current_identity(&app, &conn)?;
