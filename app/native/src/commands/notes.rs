...
fn sync_push(app: &AppContext, cmd: SyncCommand) {
...
fn spawn_blossom_deletions(app: &AppContext, blossom_deletions: Vec<(String, String)>) {
    ...
    let (keys, _) = match crate::adapters::native::key_store::keys_for_current_identity(app, &conn)
    {
...
pub fn bootstrap(app: AppContext) -> Result<BootstrapPayload, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
    Ok(NoteService::bootstrap(&repo)?)
}

pub fn todo_count(app: AppContext) -> Result<i64, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
    Ok(NoteService::todo_count(&repo)?)
}

pub fn query_notes(app: AppContext, input: NoteQueryInput) -> Result<NotePagePayload, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn load_note(app: AppContext, note_id: String) -> Result<LoadedNote, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn get_note_history(app: AppContext, note_id: String) -> Result<NoteHistoryInfo, AppError> {
    let conn = database_connection(&app)?;
...
pub async fn resolve_note_conflict(
    ...
    let (keys, _) = crate::adapters::native::key_store::keys_for_current_identity(&app, &conn)?;
    if matches!(
        action,
...
pub fn duplicate_note(app: AppContext, note_id: String) -> Result<LoadedNote, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn save_note(app: AppContext, input: SaveNoteInput) -> Result<SaveNoteResponse, AppError> {
    save_note_inner(app, input, None)
}

/// Compare and save under the same write transaction, so a remote update cannot
/// slip between the editor's stale-content check and the write.
pub fn save_note_checked(
    app: AppContext,
    input: SaveNoteInput,
    expected_markdown: &str,
) -> Result<SaveNoteResponse, AppError> {
    save_note_inner(app, input, Some(expected_markdown))
}
...
fn save_note_inner(
    ...
    let repo = SqliteNoteRepository::new(&transaction);
    if let Some(expected) = expected_markdown {
        let current = NoteService::load_note(&repo, &note_id)?;
        if current.markdown != expected || current.deleted_at.is_some() {
            return Err(AppError::custom("This note changed on another device. Your draft is still here. Save a copy to keep it."));
        }
    }
...
pub fn rename_tag(app: AppContext, input: RenameTagInput) -> Result<Vec<String>, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn delete_tag(app: AppContext, input: DeleteTagInput) -> Result<Vec<String>, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn set_tag_pinned(app: AppContext, input: SetTagPinnedInput) -> Result<(), AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn set_tag_icon(app: AppContext, input: SetTagIconInput) -> Result<(), AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn archive_note(app: AppContext, note_id: String) -> Result<LoadedNote, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn restore_note(app: AppContext, note_id: String) -> Result<LoadedNote, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn trash_note(app: AppContext, note_id: String) -> Result<LoadedNote, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn restore_from_trash(app: AppContext, note_id: String) -> Result<LoadedNote, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn delete_note_permanently(app: AppContext, note_id: String) -> Result<(), AppError> {
    let conn = database_connection(&app)?;
...
pub fn empty_trash(app: AppContext) -> Result<(), AppError> {
    let conn = database_connection(&app)?;
...
pub fn pin_note(app: AppContext, note_id: String) -> Result<LoadedNote, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn unpin_note(app: AppContext, note_id: String) -> Result<LoadedNote, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn search_notes(app: AppContext, query: String) -> Result<Vec<SearchResult>, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
    Ok(NoteService::search_notes(&repo, &query)?)
}

pub fn search_note_titles(app: AppContext, query: String) -> Result<Vec<SearchResult>, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
    Ok(NoteService::search_note_titles(&repo, &query)?)
}

pub fn search_tags(app: AppContext, query: String) -> Result<Vec<String>, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
    Ok(NoteService::search_tags(&repo, &query)?)
}

pub fn get_note_backlinks(app: AppContext, note_id: String) -> Result<Vec<NoteBacklink>, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
...
pub fn export_notes(app: AppContext, input: ExportNotesInput) -> Result<usize, AppError> {
    let conn = database_connection(&app)?;
    let repo = SqliteNoteRepository::new(&conn);
