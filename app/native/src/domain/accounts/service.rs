...
pub fn add_account(
    ...
    let creation_result = (|| -> Result<AccountSummary, AppError> {
        ...
        if store_in_keychain {
            crate::adapters::native::key_store::store_account_nsec(
                app,
                &account.public_key,
                &identity.nsec,
            )?;
    ...
    if creation_result.is_err() {
        ...
        if let Some(public_key) = stored_key_public_key.as_deref() {
            crate::adapters::native::key_store::remove_account_nsec(app, public_key);
        }
    }
...
pub fn switch_account(app: &AppContext, public_key: &str) -> Result<AccountSummary, AppError> {
...
pub fn get_account_nsec(app: &AppContext, public_key: &str) -> Result<String, AppError> {
    let conn = app_database_connection(app)?;
    let (account, _label) =
        load_account_record_by_public_key(app, &conn, public_key)?.ok_or(AccountError::NotFound)?;
    ensure_account_database_ready(&account)?;
    ...
    crate::adapters::native::key_store::load_account_nsec(app, public_key)
}

pub fn current_secret_storage_status(app: &AppContext) -> Result<SecretStorageStatus, AppError> {
...
pub fn move_current_account_nsec_to_keychain(
    ...
    crate::adapters::native::key_store::store_account_nsec(app, &account.public_key, &nsec)?;
...
pub fn rename_account(app: &AppContext, public_key: &str, name: &str) -> Result<(), AppError> {
    let conn = app_database_connection(app)?;
...
fn staged_account_dir(app: &AppContext) -> Result<PathBuf, AppError> {
