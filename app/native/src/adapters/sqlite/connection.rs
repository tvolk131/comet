...
pub fn init_database(app: &AppContext) -> Result<(), AppError> {
    let _ = ensure_active_account(app)?;
    Ok(())
}

pub fn database_connection(app: &AppContext) -> Result<Connection, AppError> {
...
pub fn app_database_connection(app: &AppContext) -> Result<Connection, AppError> {
...
pub fn app_database_path(app: &AppContext) -> Result<PathBuf, AppError> {
    let root = app_data_dir(app)?;
    Ok(root.join(APP_DATABASE_FILE))
}

pub fn active_account(app: &AppContext) -> Result<AccountRecord, AppError> {
    ensure_active_account(app)
}

pub fn active_account_dir(app: &AppContext) -> Result<PathBuf, AppError> {
    let account = ensure_active_account(app)?;
    account
        .db_path
        .parent()
...
pub fn active_account_attachments_dir(app: &AppContext) -> Result<PathBuf, AppError> {
    let dir = active_account_dir(app)?.join(ATTACHMENTS_DIR);
    fs::create_dir_all(&dir)?;
    Ok(dir)
}

fn ensure_active_account(app: &AppContext) -> Result<AccountRecord, AppError> {
...
pub(crate) fn ensure_account_database_ready(account: &AccountRecord) -> Result<(), AppError> {
    ...
    crate::adapters::sqlite::wikilink_index::ensure_wikilink_index_ready(&mut conn)?;
    Ok(())
}
...
pub(crate) fn app_data_dir(app: &AppContext) -> Result<PathBuf, AppError> {
    let config_directory = app.config_dir.as_ref().clone();
    fs::create_dir_all(&config_directory)?;
    Ok(config_directory)
}

pub(crate) fn accounts_root_dir(app: &AppContext) -> Result<PathBuf, AppError> {
...
pub(crate) fn account_db_path(app: &AppContext, npub: &str) -> Result<PathBuf, AppError> {
    Ok(app_data_dir(app)?.join(account_db_relative_path_string(npub)))
}

pub(crate) fn account_dir_for_npub(app: &AppContext, npub: &str) -> Result<PathBuf, AppError> {
