...
// ---------------------------------------------------------------------------
// Convenience functions that take AppContext (for callers not yet on the trait)
// ---------------------------------------------------------------------------

fn dir_from_app(app: &AppContext) -> Result<PathBuf, AppError> {
    crate::db::active_account_attachments_dir(app)
}

fn storage_from_app(app: &AppContext) -> Result<FsBlobStorage, AppError> {
...
pub fn import_image(app: &AppContext, source_path: &str) -> Result<ImportedImage, AppError> {
    Ok(storage_from_app(app)?.import_image(source_path)?)
}

pub fn import_image_bytes(app: &AppContext, bytes: &[u8]) -> Result<ImportedImage, AppError> {
    Ok(storage_from_app(app)?.import_image_bytes(bytes)?)
}

pub fn has_local_blob(app: &AppContext, hash: &str) -> Result<bool, AppError> {
    Ok(storage_from_app(app)?.has_blob(hash)?)
}

pub fn save_blob(app: &AppContext, hash: &str, ext: &str, data: &[u8]) -> Result<(), AppError> {
    Ok(storage_from_app(app)?.save_blob(hash, ext, data)?)
}

pub fn delete_local_blob(app: &AppContext, hash: &str) -> Result<bool, AppError> {
    Ok(storage_from_app(app)?.delete_blob(hash)?)
}

pub fn read_blob(app: &AppContext, hash: &str) -> Result<Option<(Vec<u8>, String)>, AppError> {
