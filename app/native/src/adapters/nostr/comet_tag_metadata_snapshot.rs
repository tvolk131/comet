...
impl TagMetadataSnapshotPayload {
    ...
    pub fn to_canonical_json(&self) -> Result<String, AppError> {
        serde_json::to_string(&self.canonicalized()?)
            .map_err(|e| AppError::custom(format!("Failed to serialize tag metadata payload: {e}")))
    }
...
pub fn decrypt_tag_metadata_payload(
    ...
    let json = String::from_utf8(json_bytes).map_err(|e| {
        AppError::custom(format!("Encrypted tag metadata payload is not UTF-8: {e}"))
    })?;
...
pub fn build_tag_metadata_snapshot_event(
    ...
    builder
        .sign_with_keys(keys)
        .map_err(|e| AppError::custom(format!("Failed to sign tag metadata snapshot event: {e}")))
}
...
mod tests {
    ...
    fn rejects_empty_device_id() {
        ...
        assert!(err.to_string().contains("device_id must be non-empty"));
    }
    ...
    fn rejects_unsupported_version() {
        ...
        assert!(err
            .to_string()
            .contains("Unsupported tag metadata payload version"));
    }
