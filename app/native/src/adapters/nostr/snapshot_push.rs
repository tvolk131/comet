...
fn load_access_key(app: &AppContext) -> Option<String> {
...
pub async fn push_note_snapshots_batch(
    ...
    let publish_future = send_events_to_relays_batch(
        ...
        |event_id| {
            ...
            app.notify(crate::runtime::AppEvent::SyncProgress);
        },
    );
...
async fn flush_pending_blob_uploads(app: &AppContext, keys: &Keys) -> Result<(), AppError> {
    let pubkey_hex = keys.public_key().to_hex();
...
pub async fn retry_pending_blob_uploads(app: &AppContext, keys: &Keys) -> Result<(), AppError> {
...
pub async fn push_tag_metadata_snapshot(
    ...
    {
        let mut stmt = conn
            .prepare("SELECT path, pinned, icon FROM tags WHERE pinned = 1 OR icon IS NOT NULL")?;
    ...
    let device_id = crate::domain::sync::apply_support::current_device_id(&conn)?;
    ...
    let clock_json = serialize_vector_clock(&payload.vector_clock).map_err(AppError::custom)?;
    conn.execute(
