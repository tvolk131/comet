...
pub(super) fn sync_log(_app: &AppContext, msg: &str) {
    eprintln!("[sync] {msg}");
    log::debug!("{msg}");
}
...
impl SyncManager {
    ...
    pub async fn start(&self, app: AppContext) {
        self.stop().await;
...
pub async fn auto_start(app: &AppContext) {
    if let Err(error) = start_if_ready(app).await {
        sync_log(app, &format!("failed to initialize sync: {error}"));
    }
}

pub async fn start_if_ready(app: &AppContext) -> Result<(), AppError> {
    let readiness = {
        ...
        let needs_unlock = has_relay
            ...
            && !crate::adapters::native::key_store::is_current_identity_unlocked(app, &conn)?;
...
pub(super) async fn set_state(
    ...
    *state.lock().await = new_state.clone();
    app.notify(crate::runtime::AppEvent::SyncStatus(new_state));
}
